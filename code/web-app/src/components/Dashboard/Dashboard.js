...
function setFilter(filter) {
  return (prevState, currentProps) => {
    var newState = {...prevState, searchFilter: filter};
    alert('newState='+newState.searchFilter);
    return { newState, currentProps};
  };
}
...
class Dashboard extends Component {
  ...
  doLoad() {
    ...
    //   axios.get(`/api/getsuppression` )

    axios.get(`https://psos-server.us-east.mybluemix.net/api/v1/suppression`, { crossDomain: true })
      .then(res => {
        ...
        for (var i = 0; i < csvData.length; i++) {
          ...
          if (csvDataRow.length > 5) {
            // Search officer last name, first name, badge, agency, and case number fields
            var cdrSearchables =
              csvDataRow[0].concat(", ").
                concat(csvDataRow[1]).concat(searchDelim).
                concat(csvDataRow[2]).concat(searchDelim).
                concat(csvDataRow[3]).concat(searchDelim).
                concat(csvDataRow[5]).toLowerCase();
  ...
  handleChange = (event) => {
    this.setState({searchFilter: event.target.value,}, () => (this.doLoad()));
  }
                    ...
                    <div className='Form-module--buttonRow'>
                      <div className="orderSearch">
                        <Search
                          ...
                          placeHolderText="Search officer name, badge, agency, or case #"
                          size="xl"
                          type="text"
                        /> </div>
